...
После этого он останавливается. Повторно запускайте с `FORCE=1` только для
осознанного ремонта/переустановки. Для штатного production-обновления используйте
**Настройки → Обслуживание → Обновление панели** или инструкцию
...
- 🟢 **Индикаторы клиентов** — лампочка VPN-активности по короткому Xray telemetry cache, количество устройств и OS summary из HAPP/HWID metadata
...
Автонастройка скачивает `cc-agent` из GitHub Releases этого custom-репозитория (`YLeon2007/celerity-panel-custom`). Custom-agent сохраняет совместимость с upstream endpoints `/stats`, `/sync`, `/users`, `/restart` и дополнительно отдаёт `GET /online` с VPN-online состоянием пользователей по логам Xray:

- `online=true` появляется сразу после строки Xray `accepted ... email: <userId>`;
- пользователь становится offline через 45 секунд без новых accepted-событий;
- панель опрашивает live-состояние каждые 15 секунд и использует старое traffic-delta определение только если на ноде ещё старый агент без `/online`.
